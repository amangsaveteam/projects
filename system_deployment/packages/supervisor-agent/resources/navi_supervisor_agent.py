...
class Agent:
    ...
    def module_status(self, module):
        try:
            ...
            endpoint = self.modules[module].get("endpoint", "unknown endpoint")
            return {
                "name": module, "reachable": False,
                "error": "{}: {}".format(endpoint, error), "processes": [],
            }
        return {
            "name": module,
            "reachable": True,
...
function processView(module,process){module={...module,id:escapeHtml(module.id)};process=Object.fromEntries(Object.entries(process).map(([k,v])=>[k,escapeHtml(v)]));const exit=process.exit_status===undefined?'—':process.exit_status;const error=process.spawn_error?' / '+process.spawn_error:'';return '<section class="process"><p><b>'+process.name+'</b> <span class="'+process.state+'">'+process.state+'</span></p><p class="meta">PID: '+process.pid+' · 退出码: '+exit+error+'</p><button data-module="'+module.id+'" data-process="'+process.name+'" data-action="start">启动</button><button data-module="'+module.id+'" data-process="'+process.name+'" data-action="stop">停止</button><button data-module="'+module.id+'" data-process="'+process.name+'" data-action="restart">重启</button><button data-module="'+module.id+'" data-process="'+process.name+'" data-action="log">日志尾部</button><pre id="log-'+module.id.replace(/[^a-z0-9]/gi,'_')+'-'+process.name.replace(/[^a-z0-9]/gi,'_')+'" class="logs" hidden></pre></section>'}
async function load(){try{const data=await request('/api/v1/modules');modules.replaceChildren(...data.modules.map(m=>{const a=document.createElement('article');a.innerHTML='<h2>'+escapeHtml(m.device)+' / '+escapeHtml(m.name)+'</h2>'+(!m.reachable?'<p>'+escapeHtml(m.error)+'</p>':m.processes.map(p=>processView(m,p)).join(''));a.dataset.module=m.id;return a}));attachLogs();message.textContent='已更新：'+new Date().toLocaleTimeString()}catch(e){message.textContent=e.message}}
const openLogs=new Map();
let loading=false;
function logKey(module,process){return JSON.stringify([module,process])}
function attachLogs(){for(const entry of openLogs.values()){const card=Array.from(modules.children).find(a=>a.dataset.module===entry.module);if(!card)continue;const button=Array.from(card.querySelectorAll('button[data-action="log"]')).find(b=>b.dataset.process===entry.process);(button?button.closest('.process'):card).append(entry.panel)}}
function closeLog(entry){++entry.version;openLogs.delete(logKey(entry.module,entry.process));entry.panel.remove()}
async function refreshLog(entry){const version=++entry.version;try{const data=await request('/api/v1/modules/'+encodeURIComponent(entry.module)+'/processes/'+encodeURIComponent(entry.process)+'/log?offset=0&length=32768');if(version!==entry.version)return;const output=entry.output,atEnd=output.scrollTop+output.clientHeight>=output.scrollHeight-30;const position=output.scrollTop;output.textContent=data.data||'暂无日志';output.scrollTop=atEnd?output.scrollHeight:position;entry.status.textContent=''}catch(e){if(version===entry.version)entry.status.textContent='日志读取失败：'+e.message}}
function showLog(module,process){const key=logKey(module,process);let entry=openLogs.get(key);if(!entry){const panel=document.createElement('div'),output=document.createElement('pre'),status=document.createElement('p');output.className='logs';output.textContent='正在读取日志…';entry={module,process,panel,output,status,version:0};const title=document.createElement('p');title.textContent=module+' / '+process+' 日志';panel.append(title);for(const [label,action] of [['刷新日志',()=>refreshLog(entry)],['关闭日志',()=>closeLog(entry)]]){const button=document.createElement('button');button.textContent=label;button.onclick=action;panel.append(button)}panel.append(status,output);openLogs.set(key,entry)}attachLogs();return refreshLog(entry)}
modules.onclick=async event=>{const button=event.target;if(!button.dataset.action)return;const module=button.dataset.module,process=button.dataset.process,action=button.dataset.action;try{if(action==='log'){await showLog(module,process);return}button.disabled=true;await request('/api/v1/modules/'+encodeURIComponent(module)+'/processes/'+encodeURIComponent(process)+'/'+action,{method:'POST'});await load()}catch(e){message.textContent=e.message}finally{button.disabled=false}};
setInterval(async()=>{if(!auto.checked||loading)return;loading=true;try{await load();await Promise.all(Array.from(openLogs.values(),refreshLog))}finally{loading=false}},3000);load();
</script></html>"""
