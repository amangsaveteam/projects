...
每个 Common DEB 均严格使用以下顺序：

1. `dpkg -i` 仅部署 `Middleware.env`、CycloneDDS、profile 和设备配置工具，不安装离线 payload；
2. 裸机执行 `deploy_common.py configure --target ... --robot-type ...`，写入已校验的机器人型号和设备身份；
3. 执行包内 payload 安装器。安装器会先校验设备配置，未指定有效 `ROBOT_TYPE` 时不会安装任何依赖。

总安装包也遵循同一顺序：其 system-config 会先部署环境并要求传入 `--robot-type`，然后才安装 Common DEB 和离线依赖。
...
payload 安装器在执行安装前会自动校验包内 `payloads.sha256`。包内同时含有完整依赖闭包的本地 APT 索引，但只请求 manifest 中声明的顶层包：若设备已经安装相同或更高版本，该顶层包会跳过；其余 `.deb` 只作为离线依赖候选，不能被强制安装或触发降级。因此例如设备已有更高版本的 `network-manager` 与 `libnm0` 时，两者都会保持现有版本，安装继续处理其他依赖。
